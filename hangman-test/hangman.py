...
'''
# is_word_guessed function completed successfully.
def is_word_guessed(secret_word, letters_guessed):
    ...
    '''
    letters_guessed_str = str()
    for i in letters_guessed:
        letters_guessed_str += i

    if(letters_guessed_str == secret_word):
        return True
    else:
        return False

# if you want to test this function please call function -> get_guessed_word("kindness", [k, n, d])
#already completed get_guessed_word
...
def get_available_letters(letters_guessed):
    ...
    for i in letters_guessed:
        if(letters_left.find(i) != 0):
            letters_left = letters_left.replace(i, "_")

    return letters_left

# TODO: new bug encounter when guess successfull then it will not end.
    ...
    * Display partial word guessed by the user and use underscore in place of not guess word    
    ...
    while(lives != 0):
        ...
        if letter in secret_word:
            ...
            letters_guessed.append(letter)
            lives -= 1
            print("\tLives Left : {}".format(lives),end='\n')
            if(lives == 2):
                print(IMAGES[1],end='\n\n')
            elif(lives == 1):
                print(IMAGES[4],end='\n\n')
